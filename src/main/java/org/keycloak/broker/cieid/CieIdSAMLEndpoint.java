...
public class CieIdSAMLEndpoint {
    ...
    public CieIdSAMLEndpoint(KeycloakSession session, CieIdIdentityProvider provider, CieIdIdentityProviderConfig config, UserAuthenticationIdentityProvider.AuthenticationCallback callback, DestinationValidator destinationValidator) {
    ...
    protected abstract class Binding {
            ...
            try {
                ...
                if (! isSuccessfulSamlResponse(responseType)) {
                    ...
                    if (isCieIdFault) {
                        ...
                        event.error(Errors.INVALID_SAML_RESPONSE);
                        return ErrorPage.error(session, authSession, Response.Status.BAD_REQUEST, "CieIdFault_" + responseType.getStatus().getStatusMessage().replace(' ', '_'));
                    }
                    else
                    {
                ...
                String cieIdResponseValidationError = verifyCieIdResponse(holder.getSamlDocument().getDocumentElement(), assertionElement, cieIdExpectedRequestId, requestIssueInstant, idpEntityId);
                if (cieIdResponseValidationError != null)
                {
                    ...
                    event.event(EventType.IDENTITY_PROVIDER_RESPONSE_ERROR);
                    event.error(Errors.INVALID_SAML_RESPONSE);
                    return ErrorPage.error(session, authSession, Response.Status.BAD_REQUEST, config.isDebugEnabled() ? cieIdResponseValidationError : "CieIdSamlCheck_GenericError");
                }
        ...
        private AuthenticationSessionModel samlIdpInitiatedSSO(final String clientUrlName) {
            event.event(EventType.LOGIN);
            CacheControlUtil.noBackButtonCacheControlHeader(CieIdSAMLEndpoint.this.session);
            ...
            SamlService samlService = (SamlService) factory.createProtocolEndpoint(CieIdSAMLEndpoint.this.session, event);
    ...
    private String verifyCieIdResponse(Element documentElement, Element assertionElement, String expectedRequestId, String requestIssueInstant, String idpEntityId) {
